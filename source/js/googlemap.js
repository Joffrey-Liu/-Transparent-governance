...
function drawmap(){
	...
	map.data.addListener('mouseover', function(event) {
		...
		for(j=0;j<mapdata.length;j++){
			if(event.feature.getProperty('name') == mapdata[j].key){
				for(i=0; i<companyall[id].length; i++){
					if(event.feature.getProperty('name')==companyall[id][i].Region)
						temp_name.push(companyall[id][i].Organizer);
				}
				...
				for(i=0;i<unique_temp.length;i++){
					string+=unique_temp[i]+"<hr>";
				}
				var content = "<h6>"+string+"</h6>";
				setinforwindow(event.feature.getProperty('name'), content);
				break;
			}
		}
	...
	map.data.addListener('mouseout', function(event) {
		for(i=0;i<infor_dataset.length;i++){
			infor_dataset[i].close(map);
		}
	});
}

function setinforwindow(name,string){
